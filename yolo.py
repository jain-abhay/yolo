...
class Detect(nn.Module):
    ...
    def forward(self, x):
        z_face, z_person = [], []  # inference output for face and person detection
        if self.export_cat:
            for i in range(self.nl):
                ...
                y = torch.cat([box_xy, box_wh, y[:, :, :, :, 4:5], 
                                   landm1, landm2, landm3, landm4, landm5, 
                                   y[:, :, :, :, 15:15+self.nc]], -1)

                if i == 0:  # face detection
                    z_face.append(y.view(bs, -1, self.no))
                else:  # person detection
                    z_person.append(y.view(bs, -1, self.no))

            if self.export_cat:
                return torch.cat(z_face, 1), torch.cat(z_person, 1)
        ...
        for i in range(self.nl):
            ...
            # Separating outputs for two heads
            if i == 0:  # face detection
                z_face.append(y.view(bs, -1, self.no))
            else:  # person detection
                z_person.append(y.view(bs, -1, self.no))

        return x if self.training else (torch.cat(z, 1), (torch.cat(z_face, 1), torch.cat(z_person, 1)))
...
class Model(nn.Module):
    ...
    def forward(self, x, augment=False, profile=False):
        if augment:
            ...
            y_backbone = []  # backbone outputs
            y_face, y_person = [], []  # Outputs for the two heads

            for si, fi in zip(s, f):
                xi = scale_img(x.flip(fi) if fi else x, si)
                yi_backbone, (yi_face, yi_person) = self.forward_once(xi, profile)  # forward for backbone and heads
                # Scale the bounding box coordinates back to original size
                yi_backbone[..., :4] /= si

                if fi == 2:
                    yi_backbone[..., 1] = img_size[0] - yi_backbone[..., 1]  # de-flip ud
                elif fi == 3:
                    yi_backbone[..., 0] = img_size[1] - yi_backbone[..., 0]  # de-flip lr

                y_backbone.append(yi_backbone)
                y_face.append(yi_face)
                y_person.append(yi_person)

            return torch.cat(y_backbone, 1), (torch.cat(y_face, 1), torch.cat(y_person, 1))  # augmented inference, train
        else:
            return self.forward_once(x, profile)  # single-scale inference, train


    def forward_once(self, x, profile=False):
        y_backbone, y_face, y_person = [], [], []  # Outputs for backbone and heads
        dt = []  # Profiling data

        for m in self.model:
            if m.f != -1:  # if not from previous layer
                x = y_backbone[m.f] if isinstance(m.f, int) else [x if j == -1 else y_backbone[j] for j in m.f]  # from earlier layers
            ...
            # Save output of backbone and heads
            if m.i in self.save:
                if m.i == 0:  # Backbone output
                    y_backbone.append(x)
                elif m.i == 1:  # Face detection head output
                    y_face.append(x)
                elif m.i == 2:  # Person detection head output
                    y_person.append(x)

        if profile:
            print('%.1fms total' % sum(dt))

        # Return outputs of backbone and heads
        return y_backbone[-1], (torch.cat(y_face, dim=1), torch.cat(y_person, dim=1))
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    # Parse head for face detection
    for i, (f, n, m, args) in enumerate(d['head_face']):  # from, number, module, args
        m = eval(m) if isinstance(m, str) else m  # eval strings
        for j, a in enumerate(args):
            try:
                args[j] = eval(a) if isinstance(a, str) else a  # eval strings
    ...
    # Parse head for person detection
    for i, (f, n, m, args) in enumerate(d['head_person']):  # from, number, module, args
        m = eval(m) if isinstance(m, str) else m  # eval strings
        for j, a in enumerate(args):
            try:
                args[j] = eval(a) if isinstance(a, str) else a  # eval strings
